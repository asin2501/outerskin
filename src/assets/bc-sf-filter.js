...
var bcSfFilterTemplate = {
    ...
    // Grid Template
    'productGridItemHtml': '<div class="product-grid__col">' +
                                '<div class="collection-product-card js-collection-product-card">' +
                                        '<a href="{{itemUrl}}" class="collection-product-card__image-link">{{itemImages}}</a>' +
                                    //'<div class="bc-sf-filter-product-item-label">{{itemLabels}}{{itemTagLabels}}</div>' +
                                    '<div class="collection-product-card__bottom">' +
                                        '<div class="collection-product-card__thumbnails">{{itemImagesThumb}}</div>'+
                                        '<a href="{{itemUrl}}" class="product-card__title">{{itemColelction}}{{itemTitle}}</a>' +
                                        //'{{itemVendor}}' +
                                        '{{itemPrice}}' +
                                    '</div>' +
                                '</div>' +
    ...
    'sortingHtml': '<label class="sort-by__label">Sort by:</label><select class="bc-sf-filter-filter-dropdown sort-by__select">{{sortingItems}}</select>',
    // Show Limit Template
...
BCSfFilter.prototype.buildProductGridItem = function(data, index) {
    ...
    // Add Thumbnails
    itemHtml = itemHtml.replace(/{{itemImagesThumb}}/g, buildImagesThumbnails(data));
    // Add Price
    itemHtml = itemHtml.replace(/{{itemPrice}}/g, buildPrice(data));
    itemHtml = itemHtml.replace(/{{itemColelction}}/g, buildCollection(data));
    // Add Vendor
...
function buildImages(data) {
    var html = '<div style="background-image:url(' + (images.length > 0 ? bcsffilter.optimizeImage(images[0]['src']) : bcSfFilterConfig.general.no_image_url) + ')" class="collection-product-card__img"></div>';
    return html;
}
function buildImagesThumbnails(data) {
    var html = '';
    var htmlArray = [];
    // Build Main Image
    ...
    if(images.length > 0){
        images.forEach(function(item, index){
            var thumbUrl = bcsffilter.optimizeImage(item['src']);
            htmlArray.push('<div class="collection-product-card__thubnail-col ' +
                (index == 0 ? 'collection-product-card__thubnail-col--active' : '') +
                '"><div style="background-image:url(' + thumbUrl + ')" class="collection-product-card__thubnail-img"></div></div>');
        });
        html = htmlArray.slice(0, 3).join('');
    }else{
        var html = '<div style="background-image:url(' + bcSfFilterConfig.general.no_image_url + ')" class="collection-product-card__thumbnail"></div>';
    }
    return html;
}
function buildCollection(data){
    // console.log(data);
    var collections = data.tags.filter(function(item){
        return item[0].toUpperCase() == 'E';
    });
    return '<div class="product-card__collection">'+(collections.length?collections[0]:'')+'</div>';
}
...
function buildPrice(data) {
    ...
    if (bcSfFilterConfig.custom.show_price) {
        html = '<p class="product-card__prices">';
        if (onSale) {
            html += '<span class="product-card__price product-card--selected">' + bcsffilter.formatMoney(data.price_min) + '</span>';
            html += '<span class="product-card__price product-card__price--compared">' + bcsffilter.formatMoney(data.compare_at_price_min) + '</span> ';
        } else {
            if (priceVaries) {
                html += (bcSfFilterConfig.label_basic.from) + ' ';
            }
            html += '<span class="product-card__price">' + bcsffilter.formatMoney(data.price_min) + '</span>';
        }
        html += '</p>';
    }
    return html;
    ...
    if (showall) {
        ...
        if (data.tags) {
            for (i = 0; i < data.tags.length; i++) { 
                tag = data.tags[i];
                if(tag.includes("pfs:label")){
                    preTagLabel = bcSfFilterTemplate.tagLabelHtml.replace(/{{labelTag}}/g, tag.split('pfs:label-')[1]);
